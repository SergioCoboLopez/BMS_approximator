...
As a starting point, the user should run the notebook `generate_ANN_datasets.ipynb` :
The code is intended to be run only once, as it generates all data needed for the experiments.
...
The last two cells are intended to generate figures of the functions, but are under work yet.
...
`train_anns.py`: This code creates and trains a single neural network using the Levenberg-Marquadt algorithm from the [pyrenn](https://pyrenn.readthedocs.io/en/latest/index.html) Python library 