...
#----------------------------------
input_path= '../../data/out_of_rank/'
file_name_d=input_path + 'NN_out_rank_function_'+function + '_NREP_10_res_' + str(resolutions[resolution]) + '_data.csv'

output_path='../../data/nns/' + resolution + '_resolution/out_of_rank/'


d=pd.read_csv(file_name_d)
d=d.drop(columns='Unnamed: 0')

d=d[(d['x1'] >= -2.0) & (d['x1']<=14.0)]
...
d.to_csv( output_path + 'NN_out_of_rank_' + function + '_sigma_' + str(sigma) + '_r_' + str(realization) + '_res_'  + resolutions[resolution] + '.csv')
